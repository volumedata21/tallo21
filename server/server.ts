  ...
  try {
    const transaction = db.transaction((targetIds) => {
      if (updateKeys.length > 0) {
        ...
        for (const id of targetIds) stmt.run({ ...allowedUpdates, id });
      }
...
app.post('/api/images', upload.single('file'), (req, res) => {
  try {
    ...
    res.json({ ...newImage, isFavorite: Boolean(newImage.isFavorite) });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
});
...
app.get('/api/boards', (req, res) => {
  try {
    ...
    res.json(rows.map(r => ({ 
      ...r, 
      collectionIds: JSON.parse(r.collection_ids || '[]'), 
      createdAt: r.created_at, 
      ownerId: r.owner_id,
      coverImageId: r.cover_image_id // FIX: Map database column to API property
    })));
  } catch (err) { res.json([]); }
});
...
app.post('/api/boards', (req, res) => {
  ...
  try {
    // FIX: Safely stringify collectionIds and include coverImageId
    const collectionIdsStr = JSON.stringify(b.collectionIds || []);
    
    db.prepare(`
      INSERT OR REPLACE INTO boards (id, name, description, created_at, owner_id, collection_ids, visibility, cover_image_id) 
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      b.id, 
      b.name, 
...
// --- API: Other (Collections, Groups, Discovery) ---
// (Kept standard, just ensuring stability)
