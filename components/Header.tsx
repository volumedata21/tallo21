...
import { Search, Hash, Map, Grid, Plus, Menu, Settings, Shield, ChevronDown, LogOut, Globe, LogIn, X } from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({
  ...
  onOpenAdmin, onLogout, onOpenSettings, onLoginClick
}) => {
  ...
  return (
    <header className="h-16 md:h-20 bg-[#000208]/90 backdrop-blur-md border-b border-slate-800 flex items-center justify-between px-4 sm:px-8 z-30 sticky top-0 transition-all pt-[env(safe-area-inset-top)]">
      {/* MOBILE SEARCH OVERLAY */}
      {isMobileSearchOpen && (
        <div className="absolute inset-0 bg-[#000208] z-50 flex items-center px-4 gap-2 md:hidden animate-in fade-in slide-in-from-top-2">
          ...
          <input
            ...
            className="flex-1 bg-transparent border-none text-white focus:ring-0 outline-none text-base" // text-base prevents iOS zoom
          />
      ...
      <div className="flex items-center gap-2 md:gap-4 flex-1">
        {user && (
          <button onClick={onToggleSidebar} className="p-2 -ml-2 text-slate-400 hover:text-white md:hidden active:scale-95 transition-transform">
            <Menu size={24} />
          </button>
        )}

        {/* Mobile Search Trigger */}
        <button onClick={() => setIsMobileSearchOpen(true)} className="p-2 text-slate-400 hover:text-white md:hidden">
          <Search size={20} />
        </button>
      ...
      <div className="flex items-center gap-2 sm:gap-4">
        ...
        <div className="flex bg-slate-900 rounded-lg p-1 border border-slate-800">
          <button onClick={() => onToggleView('grid')} className={`p-1.5 md:p-2 rounded-md transition-all ${viewMode === 'grid' ? 'bg-slate-800 text-teal-400 shadow-sm' : 'text-slate-500 hover:text-slate-300'}`}>
            <Grid size={18} />
          </button>
          <button onClick={() => onToggleView('map')} className={`p-1.5 md:p-2 rounded-md transition-all ${viewMode === 'map' ? 'bg-slate-800 text-teal-400 shadow-sm' : 'text-slate-500 hover:text-slate-300'}`}>
            <Map size={18} />
          </button>
        </div>
        ...
        {user ? (
          <>
            <button onClick={onCreatePin} className="flex items-center gap-2 bg-teal-600 hover:bg-teal-500 text-white p-2 md:px-4 md:py-2 rounded-full font-bold transition-all shadow-lg shadow-teal-900/20 active:scale-95" title="Create Stem">
              <Plus size={20} />
          ...
          <button onClick={onLoginClick} className="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 text-white px-4 py-2 rounded-full font-bold transition-all border border-slate-700">
            <LogIn size={16} />
